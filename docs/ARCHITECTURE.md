...
#### 2. *Remove the need to manually push an initial image to ECR*
Terraform doesn't initialize the ECS application correctly if there's no image in ECR yet, so during initial setup you currently need to push a locally built image to ECR. Find a workaround to remove this requirement.

#### 3. *Replace personal access token use for creating build PRs*
Because the repo settings do not allow GitHub Actions to directly create PRs and modern scoped PATs can't be used on private repos you don't own, I'm using a classic PAT to handle the creation of automated build deployment PRs. It's stored in a secret that can't be directly viewed, but anyone else with repo access could use custom actions to misuse my credentials. I trust that no one here would do that, but security should not depend on trust alone. Long-term, I'd work out a way to use more carefully scoped modern PATs, set up a GitHub application to handle PR creation, configure the repo to allow GitHub Actions to create PRs, or use a GitHub Environment to manage deployment instead of a PR-based approach.

#### 4. *Set up a registered domain*
...
#### 5. *Modularize terraform files and GitHub actions for multiple environments.*
...
#### 6. *Set up a separate developer test environment.*
...
#### 8. *Add linting, testing, and security scans.*
...
#### 9. *Set up a canary deployment pattern.*
